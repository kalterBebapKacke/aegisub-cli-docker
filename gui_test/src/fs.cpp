...
namespace {
	...
	// Custom Remove function to handle non-empty directories
	bool Remove(path const& p) {
		boost::system::error_code ec;

		// First try simple remove (works for files and empty directories)
		bool result = bfs::remove(p, ec);

		if (ec.value() == boost::system::errc::directory_not_empty) {
			// If directory is not empty, use remove_all for recursive removal
			std::uintmax_t removed_count = bfs::remove_all(p, ec);
			if (ec.value() == boost::system::errc::success) {
				LOG_D("filesystem") << "Recursively removed " << removed_count << " items from: " << p.string();
				return removed_count > 0;
			}
		}

		// Handle other errors with the existing error checking
		switch (ec.value()) {
			...
			case boost::system::errc::permission_denied:
				acs::CheckFileRead(p);
				throw AccessDenied(p);
			default:
				LOG_D("filesystem") << "Unknown error when calling remove: " << ec << ": " << ec.message();
				throw FileSystemUnknownError(ec.message());
		}
	}
